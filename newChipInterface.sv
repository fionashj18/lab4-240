...
module ChipInterface (
  ...
  logic loadShape_sync, gradeIt_sync, startGame_sync, coin_sync;
 ...
 */
    ...
    MastermindVGA mmVGA(.clk_40MHz,
                        ...
                        .znarly(Znarly),
                        .zood(Zood),
                        .clearGame(clr | startGame_sync),
                        .masterPattern(masterPattern),
                        .displayMasterPattern(1'b1),
                        // .displayMasterPattern(SW[13]),
                        .loadZnarlyZood(gradeIt_sync & CanGrade)
                       );
   
    EightSevenSegmentDisplays display (.HEX7(4'd0), 
                                       .HEX6(4'd0), 
                                       .HEX5(4'd0), 
                                       .HEX4(4'd0),
                                       .HEX3(Znarly),
    ...
    Synchronizer loadsync(.async(BTN[3]), 
                             .clock(CLOCK_100), 
                             .sync(loadShape_sync)
                            );   
    Synchronizer gradesync(.async(BTN[3]), 
                                .clock(CLOCK_100), 
                                .sync(gradeIt_sync)
                                );  
    Synchronizer start_sync(.async(BTN[2]), 
                             .clock(CLOCK_100), 
                             .sync(startGame_sync)
                            );   
    Synchronizer coinsync(.async(BTN[1]), 
                             .clock(CLOCK_100), 
                             .sync(coin_sync)
                            );                  
